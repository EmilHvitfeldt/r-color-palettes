...
fs::dir_create("readme_images")

readme_pals <- paletteer::palettes_d
readme_pals <- readme_pals[names(readme_pals) != "palettetown"]

walk2(readme_pals, names(readme_pals), make_plot)

pkg_data <- paletteer::paletteer_packages |>
  filter(Name %in% names(readme_pals))

github_install <- if_else(is.na(pkg_data$github_ver), "", glue::glue("
# Developmental version
pak::pak(\"{pkg_data$Github}\")"))

cran_install <- if_else(pkg_data$CRAN, "", glue::glue("

# CRAN version
install.packages(\"{pkg_data$Name}\")

"))

glue::glue_data(pkg_data, "
### [[Name]]
